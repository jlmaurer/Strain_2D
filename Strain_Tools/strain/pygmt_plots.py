...
def plot_dilatation(dila_array, station_vels, region, outdir, outfile, positive_eigs=(), negative_eigs=(), peak_dil=75):
    ...
    fig = pygmt.Figure()
    scalesize = get_map_scale(region)
    pygmt.makecpt(cmap="polar", series=f"-{peak_dil}/{peak_dil}/2", reverse=True, background="o", output=outdir+"/mycpt.cpt")
...
def plot_maxshear(max_shear_array, station_vels, region, outdir, outfile, positive_eigs=(), negative_eigs=(),peak_max_shear=200):
    ...
    fig = pygmt.Figure()
    scalesize = get_map_scale(region)
    pygmt.makecpt(cmap="polar", series=f"0/{peak_max_shear}/2", truncate="0/1.0", background="o", output=outdir+"/mycpt.cpt")
    ...
    fig.colorbar(position="JCR+w4.0i+v+o0.7i/0i", cmap=outdir+"/mycpt.cpt", truncate=f"0/{peak_max_shear}", frame=["x50", "y+L\"Nanostr/yr\""])
